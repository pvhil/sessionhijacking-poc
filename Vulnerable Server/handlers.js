// Copy of https://github.com/sohamkamani/nodejs-session-cookie-example
// This File has all the orginal code and comments from the original author, so you can understand it better later on
// New Comments by my are marked with // NEW
...
const welcomeHandler = (req, res) => {
  ...
  // NEW
  // What happens if the user changes his IP address?
  // Right nothing, because we don't check it
  // This is a vulnerability, because the user can use the same session token from another device
  // and the server will think it is the same user
  // This can be exploited by an attacker, who can use the session token to access the users account
  // The attacker can get the session token by numerous ways, for example by using a XSS attack
  // or by sniffing the network traffic
  // This is very common, for example numerous websites have been hacked by this method
  // Especially YouTubers like Linus Tech Tips, who has been hacked by this method
  // NEW end
...
const refreshHandler = (req, res) => {
  ...
  // NEW
  // Again, no IP Check
  // also vulnerable to the same attack as the welcomeHandler
  // NEW end
